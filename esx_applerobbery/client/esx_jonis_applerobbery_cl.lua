						...
						if IsPedShooting(GetPlayerPed(-1)) then
							if Config.NeedBag then
							    if  borsa ~= -1 and borsa ~= 0 then
							        ESX.TriggerServerCallback('esx_applerobbery:conteggio', function(CopsConnected)
	...
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 152.3919, -225.792, 54.424, true) <= 10 and not blip then
			...
			if GetDistanceBetweenCoords(coords, 152.3919, -225.792, 54.424, true) < 1.0 then
				...
				ESX.ShowFloatingHelpNotification("Presiona ~y~E~s~ para vender las ~p~joyas",vector3(706.669, -966.898, 30.00+1))
				if IsControlJustReleased(1, 51) then
					blip = true
					ESX.TriggerServerCallback('esx_vangelico:getItemAmmount', function(quantity)
